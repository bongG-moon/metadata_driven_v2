...
현재 v2 main flow의 `02 Metadata Context Loader`는 MongoDB에서 아래 세 collection을 읽는다.

| Metadata | Collection | Writer 기준 key | Main flow에서 조립되는 위치 |
| --- | --- | --- | --- |
| Domain | `agent_v2_domain_items` 또는 입력한 full collection name | `section + key` 또는 `gbn + key` | `metadata.domain_items` |
| Table catalog | `agent_v2_table_catalog_items` 또는 입력한 full collection name | `dataset_key` | `metadata.table_catalog.datasets` |
| Main flow filter | `agent_v2_main_flow_filters` 또는 입력한 full collection name | `filter_key` | `metadata.main_flow_filters` |

기본값은 `database=metadata_driven_agent_v2`, `domain_collection_name=agent_v2_domain_items`, `table_catalog_collection_name=agent_v2_table_catalog_items`, `main_flow_filter_collection_name=agent_v2_main_flow_filters`다. 작업자가 다른 collection에 저장/조회해야 할 때도 prefix가 아니라 collection full name을 그대로 입력한다.
...
기존 repo의 authoring flow는 `gbn` 이름을 사용했지만, v2 loader는 `section` 또는 `gbn` 모두 읽을 수 있게 두는 것이 좋다. 새 구현에서는 사용자와 문서에는 `section`을 쓰고, 호환을 위해 writer가 `gbn`도 선택적으로 받을 수 있게 한다.
...
```json
{
  ...
  "ready_to_save": true,
  "saved_count": 1,
  "target": "metadata_driven_agent_v2.agent_v2_table_catalog_items",
  "keys": ["wip_today"],
