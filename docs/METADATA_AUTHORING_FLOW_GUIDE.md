...
| --- | --- | --- | --- |
| Domain | `{prefix}_domain_items` | `section + key` 또는 `gbn + key` | `metadata.domain_items` |
| Table catalog | `{prefix}_table_catalog_items` | `dataset_key` | `metadata.table_catalog.datasets` |
| Main flow filter | `{prefix}_main_flow_filters` | `filter_key` | `metadata.main_flow_filters` |

기본값은 `database=metadata_driven_agent_v2`, `collection_prefix=agent_v2`다.
