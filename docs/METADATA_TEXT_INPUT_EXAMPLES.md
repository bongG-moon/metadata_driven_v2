...
계열별 source alias는 production=production_data, wip=wip_data, target=target_data로 봅니다.
...
Hold Lot처럼 row 목록으로 보여 달라는 용어는 집계하지 않는 상세 row 모드로 저장합니다.
...
- 분석 recipe의 `grain_policy`, `source_aliases_by_family`가 저장 문서에 유지되는지
- 상세 row 용어의 `result_mode=detail_rows`와 table catalog의 `default_detail_columns`가 저장 문서에 유지되는지
