...
| 01 | 01 MongoDB Data Loader | `langflow_components/main_flow/01_mongodb_data_loader.py` | 이전 state의 compact `data_ref`를 preview/summary로 가볍게 복원 |
| 02 | 02 Metadata Context Loader | `langflow_components/main_flow/02_metadata_context_loader.py` | MongoDB metadata 로드 |
| 03 | 03 Intent Prompt Builder | `langflow_components/main_flow/03_intent_prompt_builder.py` | intent JSON 생성을 위한 LLM prompt 생성 |
| LLM-A | Gemini/LLM Intent JSON | Langflow 기본 Gemini/LLM node | intent JSON 생성 |
| 04 | 04 Intent Plan Normalizer | `langflow_components/main_flow/04_intent_plan_normalizer.py` | intent JSON 정규화, retrieval_jobs 생성/보강 |
| 04B | 01 MongoDB Data Loader | `langflow_components/main_flow/01_mongodb_data_loader.py` | `hydrate_mode=auto`로 필요한 후속 분석만 이전 결과 전체 rows 복원 |
| Retrieval | Data Retrieval Flow | 아래 2장 참고 | source_type별 데이터 조회 |
| 05 | 05 Retrieval Payload Adapter | `langflow_components/main_flow/05_retrieval_payload_adapter.py` | retrieval 결과를 `runtime_sources`와 compact `source_results`로 변환 |
| 06 | 06 Pandas Prompt Builder | `langflow_components/main_flow/06_pandas_prompt_builder.py` | pandas code JSON 생성을 위한 LLM prompt 생성 |
| LLM-B | Gemini/LLM Pandas Code JSON | Langflow 기본 Gemini/LLM node | pandas code JSON 생성 |
| 07 | 07 Pandas Code Executor | `langflow_components/main_flow/07_pandas_code_executor.py` | safety check 후 pandas code 실행 |
| 08 | 08 MongoDB Data Store | `langflow_components/main_flow/08_mongodb_data_store.py` | source `runtime_sources`와 pandas `analysis.rows`를 MongoDB result collection에 저장하고 `data_ref`로 축약 |
...
| 4 | 00 Request State Loader | `payload` | 01 MongoDB Data Loader | `payload` | 이전 state의 compact `data_ref` preview/summary 복원 |
| 5 | 01 MongoDB Data Loader | `payload_out` | 02 Metadata Context Loader | `payload` | metadata 로드 전 가벼운 state 복원 |
...
| 9 | Text Input | value | 08 MongoDB Data Store, 01 MongoDB Data Loader | `result_collection_name` | full collection name 입력. 기본 `agent_v2_result_store` |
| 10 | 02 Metadata Context Loader | `payload_out` | 03 Intent Prompt Builder | `payload` | metadata 포함 payload |
| 11 | 03 Intent Prompt Builder | `intent_prompt` | LLM-A Gemini/LLM Intent JSON | prompt/message input | JSON-only 응답 권장 |
| 12 | 02 Metadata Context Loader | `payload_out` | 04 Intent Plan Normalizer | `payload` | LLM 응답과 결합할 payload |
| 13 | LLM-A Gemini/LLM Intent JSON | text/message output | 04 Intent Plan Normalizer | `llm_response` | intent JSON 응답 |
| 14 | 04 Intent Plan Normalizer | `payload_out` | 01 MongoDB Data Loader second instance | `payload` | `hydrate_mode=auto` |
| 15 | second 01 MongoDB Data Loader | `payload_out` | Data Retrieval Flow start node(s) | `payload` | full hydrate 반영 payload |
| 16 | second 01 MongoDB Data Loader | `payload_out` | 05 Retrieval Payload Adapter | `main_payload` | main payload branch |
| 17 | Data Retrieval Flow end | `retrieval_payload` | 05 Retrieval Payload Adapter | `retrieval_payload` | source 조회 결과 병합 |
| 18 | 05 Retrieval Payload Adapter | `payload` | 06 Pandas Prompt Builder | `payload` | pandas prompt 생성 |
| 19 | 06 Pandas Prompt Builder | `pandas_prompt` | LLM-B Gemini/LLM Pandas Code JSON | prompt/message input | JSON-only 응답 필수 |
| 20 | 05 Retrieval Payload Adapter | `payload` | 07 Pandas Code Executor | `payload` | code 실행용 source rows payload |
| 21 | LLM-B Gemini/LLM Pandas Code JSON | text/message output | 07 Pandas Code Executor | `llm_response` | pandas code JSON 응답 |
| 22 | 07 Pandas Code Executor | `payload_out` | 08 MongoDB Data Store | `payload` | source/result rows compact |
| 23 | 08 MongoDB Data Store | `payload_out` | 09 Answer Prompt Builder | `payload` | 최종 답변 prompt 생성 |
...
| 25 | 08 MongoDB Data Store | `payload_out` | 10 Answer Response Builder | `payload` | 답변/state 조립용 payload |
| 26 | LLM-C Gemini/LLM Final Answer | text/message output | 10 Answer Response Builder | `llm_response` | 최종 답변 |
| 27 | 10 Answer Response Builder | `payload_out` | 11 Answer Message Adapter | `payload` | Playground 출력용 message 생성 |
| 28 | 11 Answer Message Adapter | `message` | Chat Output | `message` | 사용자에게 보일 최종 출력 |
| 29 | 10 Answer Response Builder | `payload_out.state` | State Store | stored state | 다음 질문의 00 `state` input으로 재사용 |
...
`01 MongoDB Data Loader`의 앞단 기본 `hydrate_mode`는 `preview`입니다. 이 모드에서는 이전 `state.current_data`의 preview rows, `row_count`, `columns`, `data_ref`, `product_key_values`만 후속 계획에 사용하고 전체 rows를 payload 초반에 붙이지 않습니다. 04 이후의 두 번째 loader 인스턴스는 `hydrate_mode=auto`로 두며, `intent_plan.requires_full_state_hydrate=true`일 때만 full hydrate합니다.
...
| --- | --- | --- | --- | --- |
| D1 | 04 Intent Plan Normalizer | `payload_out` | 01 Dummy Data Retriever | `payload` |
| D2 | 01 Dummy Data Retriever | `retrieval_payload` | 05 Retrieval Payload Adapter | `retrieval_payload` |
...
| # | From node | From output | To node | To input |
| --- | --- | --- | --- | --- |
| R1 | 04 Intent Plan Normalizer | `payload_out` | 02 Oracle Query Retriever | `payload` |
| R2 | 04 Intent Plan Normalizer | `payload_out` | 03 H-API Retriever | `payload` |
| R3 | 04 Intent Plan Normalizer | `payload_out` | 04 Datalake Retriever | `payload` |
| R4 | 04 Intent Plan Normalizer | `payload_out` | 05 Goodocs Retriever | `payload` |
...
| R9 | 06 Source Retrieval Merger | `retrieval_payload` | 05 Retrieval Payload Adapter | `retrieval_payload` |
