...
- `question`: 현재 사용자 질문
- `session_id`: 대화 세션 키
- `state`: 명시 state. 값이 있으면 MongoDB보다 우선합니다.
- `mongo_uri`: 비워두면 `MONGODB_URI` 또는 `MONGO_URI`를 사용합니다.
- `mongo_database`: 기본 `metadata_driven_agent_v2`
- `session_collection_name`: 기본 `agent_v2_session_states`
...
$env:MONGODB_DATABASE="metadata_driven_agent_v2"
$env:MONGODB_SESSION_STATE_COLLECTION="agent_v2_session_states"
$env:SESSION_STATE_PREVIEW_ROW_LIMIT="5"
$env:SESSION_STATE_HISTORY_LIMIT="10"
```

If `state` is explicitly passed to `LangflowApiClient.run_query`, that state is used first. If `state=None`, the client loads state by `session_id` and saves the response state after the call.
