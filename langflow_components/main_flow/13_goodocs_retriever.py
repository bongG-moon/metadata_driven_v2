...
def _run_goodocs_job(
    ...
    if not any(str(value or "").strip() for value in credentials.values()):
        rows = _apply_filters(_dummy_rows(job, doc_id), job)[:fetch_limit]
    ...
    try:
        ...
        rows = _apply_filters(_frame_to_rows(frame), job)[:fetch_limit]
...
def _apply_filters(rows: list[dict[str, Any]], job: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in filters:
        ...
        if op == "eq":
            ...
            result = [row for row in result if str(row.get(field) or "") == expected]
        elif op == "in":
            values = {str(value or "") for value in item.get("values", []) if str(value or "")}
            result = [row for row in result if str(row.get(field) or "") in values]
        elif op == "not_empty":
            result = [row for row in result if row.get(field) not in (None, "")]
    return result
...
class GoodocsRetriever(Component):
    ...
    display_name = "13 Goodocs Retriever"
