...
def test_mongodb_upload_optional_batches_include_regression_and_samples():
    module = _load_upload_module()
    batches = module.build_upload_batches(ROOT, "agent_v2", include_regression=True, include_sample_data=True)
...
def test_mongodb_upload_docs_have_deterministic_ids():
    module = _load_upload_module()
    first = module.build_upload_batches(ROOT, "agent_v2", include_sample_data=True)
    second = module.build_upload_batches(ROOT, "agent_v2", include_sample_data=True)
